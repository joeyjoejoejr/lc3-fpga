...
module lc3_str_tb;
  ...
  initial begin
    $readmemh("programs/str/str_smoke.hex", memory.mem);
  end

  initial begin
    if ($test$plusargs("dump")) begin
      $dumpfile("sim/lc3_str_tb.vcd");
      $dumpvars(0, lc3_str_tb);
    end
    ...
    for (cycle = 0; cycle < 200; cycle = cycle + 1) begin
      ...
      if (dut.halted) begin
        ...
        if (memory.mem[16'h3008] !== 16'h0007 ||
        ...
        print_case_pass("str_smoke");
        $finish;
      end
    end

    if (!saw_halt) begin
      $display("%s[FAIL]%s %-14s CPU did not halt", TB_RED, TB_RESET, "str_smoke");
      $fatal(1);
    end
  end
endmodule
